...
则问题转化成：
$$ \begin{aligned} \max_{\boldsymbol{\alpha}} \quad & \sum_{i=1}^{m} \alpha_i - \frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\alpha_i\alpha_j y_i y_j \kappa(\boldsymbol{x}_i,\boldsymbol{x}_j) \\ \text{s.t.} \quad & \sum_{i=1}^{m} \alpha_i y_i = 0, \\ & \alpha_i \ge 0,\quad i=1,2,\dots,m . \end{aligned} $$
### 常见核函数
...
如果视觉上看数据近似线性可分，则采用线性核
否则，对于绝大多数未知分布数据集优先试高斯核
## Soft Margin

由于样本中可能存在噪声，要求超平面在零错误的情况下将样本完全分类肯会造成过拟合
所以可以放宽条件（允许分类出现错误）
$$ \min_{\boldsymbol{w},b} \quad \frac{1}{2}\|\boldsymbol{w}\|^2 + C\sum_{i=1}^{m}\ell_{0/1}\big(y_i(\boldsymbol{w}^\mathrm{T}\boldsymbol{x}_i + b)-1\big) $$
其中 $\ell_{0/1}(z)= \begin{cases} 1, & \text{if } z < 0; \\ 0, & \text{otherwise}. \end{cases}$
# 回归问题
/
与传统的回归模型不同，支持向量机回归仅当 $f(\boldsymbol{x})$ 与 $y$ 的差别的绝对值大于 $\epsilon$ 时才计算损失
$$ \min_{\boldsymbol{w},b} \quad \frac{1}{2}\|\boldsymbol{w}\|^2 + C\sum_{i=1}^{m}\ell_{\epsilon}\big(f(\boldsymbol{x}_i)-y_i\big), $$其中 $\ell_{\epsilon}(z)= \begin{cases} 0, & \text{if } |z| \le \epsilon; \\ |z| - \epsilon, & \text{otherwise}. \end{cases}$
